...
CIntroScene::CIntroScene(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIObject{ pDevice, pContext }
{
}

CIntroScene::CIntroScene(const CIntroScene& Prototype)
	: CUIObject(Prototype)
{
}

HRESULT CIntroScene::Initialize_Prototype()
{
...
HRESULT CIntroScene::Initialize(void* pArg)
{
...
void CIntroScene::Priority_Update(_float fTimeDelta)
{
	__super::Priority_Update(fTimeDelta);
}

void CIntroScene::Update(_float fTimeDelta)
{
...
void CIntroScene::Late_Update(_float fTimeDelta)
{
	__super::Late_Update(fTimeDelta);
	m_pGameInstance->Add_RenderGroup(CRenderer::RG_FONT, this);
}

HRESULT CIntroScene::Render()
{
	if (m_bRenderOpen)
	{
...
HRESULT CIntroScene::Ready_UIObject()
{
	LoadData_UI_Scene(UISCENE_INTRO, L"UIScene_Intro");
	//LoadData_Text_Scene();
	return S_OK;
}

HRESULT CIntroScene::LoadData_UI_Scene(_uint iSceneIndex, const _tchar* szSceneName)
{
...
HRESULT CIntroScene::LoadData_Text_Scene()
{
...
CIntroScene* CIntroScene::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CIntroScene* pInstance = new CIntroScene(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CIntroScene");
...
CGameObject* CIntroScene::Clone(void* pArg)
{
	CIntroScene* pInstance = new CIntroScene(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CIntroScene");
		Safe_Release(pInstance);
	}
